...
public class MeteoResource {
    ...
    public Response list() {
    	...
    	DAOMeteo met = new DAOMeteo(em);
    	
    	res = Response.ok(met.findAll());
    ...
    public Response findById(@PathParam("id") String arg0) {
    	...
    	res = Response.ok(met.find(Integer.parseInt(arg0)));
    	res.header("Access-Control-Allow-Origin", "*");
    	res.header("Access-Control-Allow-Methods", "POST, GET, PUT, UPDATE, OPTIONS");
    	res.header("Access-Control-Allow-Headers", "Content-Type, Accept, X-Requested-With");
    ...
    @PUT @Path("update/{id}")
    @Produces({MediaType.APPLICATION_JSON})
    public Response updateById(Meteo u) {
    	EntityManagerFactory emf = Persistence.createEntityManagerFactory("jpa");
    	EntityManager em = emf.createEntityManager();
    	
    	DAOMeteo met = new DAOMeteo(em);
    	
    	res = Response.ok(met.update(u));
    	res.header("Access-Control-Allow-Origin", "*");
    	res.header("Access-Control-Allow-Methods", "POST, GET, PUT, UPDATE, OPTIONS");
    	res.header("Access-Control-Allow-Headers", "Content-Type, Accept, X-Requested-With");
    ...
    @PUT @Path("add")
    @Produces({MediaType.APPLICATION_JSON})
    public Response add(Meteo u) {
    	EntityManagerFactory emf = Persistence.createEntityManagerFactory("jpa");
    	EntityManager em = emf.createEntityManager();
    	
    	DAOMeteo met = new DAOMeteo(em);
    	
    	res = Response.ok(met.insert(u));
    	res.header("Access-Control-Allow-Origin", "*");
    	res.header("Access-Control-Allow-Methods", "POST, GET, PUT, UPDATE, OPTIONS");
    	res.header("Access-Control-Allow-Headers", "Content-Type, Accept, X-Requested-With");
